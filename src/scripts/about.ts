...
// 笑脸用内联 data-URI（源站是 /images/asset-smiley--*.svg，资源已丢失；老项目方案自包含）
...
/* s-about：边框内容盒滚动视差 + 网格线（外框→内容盒角点连线，滚动进度驱动）
   + 奖项 hover 笑脸彩纸。机制移植自 source.js About 类（老项目已验收实现） */
export class About {
  ...
  private bindEvents() {
    ...
    // 奖项格入场揭示（mask 右收）+ hover/touch 触发按压与彩纸
  ...
  // 网格：外圈→内容盒的透视连线（竖列/横排 + 角落插值），内容盒位移由滚动进度模拟
  private setLines() {
    const b = this.bounding;
    this.lines = [];
